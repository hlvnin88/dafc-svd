"""F9: GraphCodeBERT 768 → 256."""
import numpy as np
import torch
import torch.nn as nn
from config import GRAPHCODEBERT_NAME, ENCODER_MAX_LEN, FUSION_DIM, DATA_DIR, DEVICE, MODELS_DIR
...
def _load():
    ...
    if _tok is None:
        ...
        _tok = AutoTokenizer.from_pretrained(GRAPHCODEBERT_NAME)
        _mdl = AutoModel.from_pretrained(GRAPHCODEBERT_NAME).eval().to(DEVICE)
...
def extract_f9(tr_codes, va_codes, te_codes, name, y_tr):
    X_tr = _embed768(tr_codes); X_va = _embed768(va_codes); X_te = _embed768(te_codes)
    proj = _train_projection(X_tr, y_tr)
    Z_tr = _apply(proj, X_tr); Z_va = _apply(proj, X_va); Z_te = _apply(proj, X_te)
    out = ensure_dir(DATA_DIR / name / 'features' / 'F9')
    np.save(out / f'{name}_train_f9.npy', Z_tr)
    np.save(out / f'{name}_val_f9.npy',   Z_va)
    np.save(out / f'{name}_test_f9.npy',  Z_te)
    torch.save(proj.state_dict(),
               ensure_dir(MODELS_DIR / 'encoders') / f'{name}_f9_proj.pth')
